...
public class CloudRecordController {
    ...
    public List<String> openRtpServer(
        ...
        log.info("[云端录像] 查询存在云端录像的日期 app->{}, stream->{}, mediaServerId->{}, year->{}, month->{}", app, stream,
                mediaServerId, year, month);
    ...
    public PageInfo<CloudRecordItem> openRtpServer(@RequestParam(required = false) String query,
        ...
        log.info(
                "[云端录像] 查询 app->{}, stream->{}, mediaServerId->{}, page->{}, count->{}, startTime->{}, endTime->{}, callId->{}",
    ...
    public String addTask(HttpServletRequest request, @RequestParam(required = false) String app,
            @RequestParam(required = false) String stream, @RequestParam(required = false) String mediaServerId,
            @RequestParam(required = false) String startTime, @RequestParam(required = false) String endTime,
            @RequestParam(required = false) String callId, @RequestParam(required = false) String remoteHost) {
        ...
        if (mediaServer == null) {
            ...
            if (remoteHost == null) {
                remoteHost = request.getScheme() + "://" + mediaServer.getIp() + ":"
                        + mediaServer.getRecordAssistPort();
            }
        }
        return cloudRecordService.addTask(app, stream, mediaServer, startTime, endTime, callId, remoteHost,
                mediaServerId != null);
    }
    ...
    public JSONArray queryTaskList(HttpServletRequest request, @RequestParam(required = false) String app,
            @RequestParam(required = false) String stream, @RequestParam(required = false) String callId,
            @RequestParam(required = false) String taskId, @RequestParam(required = false) String mediaServerId,
    ...
    public int addCollect(@RequestParam(required = false) String app, @RequestParam(required = false) String stream,
            @RequestParam(required = false) String mediaServerId, @RequestParam(required = false) String startTime,
            @RequestParam(required = false) String endTime, @RequestParam(required = false) String callId,
            @RequestParam(required = false) Integer recordId) {
        log.info("[云端录像] 添加收藏，app={}，stream={},mediaServerId={},startTime={},endTime={},callId={},recordId={}", app,
                stream, mediaServerId, startTime, endTime, callId, recordId);
        if (recordId != null) {
    ...
    public int deleteCollect(@RequestParam(required = false) String app, @RequestParam(required = false) String stream,
            @RequestParam(required = false) String mediaServerId, @RequestParam(required = false) String startTime,
            @RequestParam(required = false) String endTime, @RequestParam(required = false) String callId,
            @RequestParam(required = false) Integer recordId) {
        log.info("[云端录像] 移除收藏，app={}，stream={},mediaServerId={},startTime={},endTime={},callId={},recordId={}", app,
                stream, mediaServerId, startTime, endTime, callId, recordId);
        if (recordId != null) {
    ...
    public DeferredResult<WVPResult<StreamContent>> loadRecord(
        ...
        log.info("[控制器-云录像加载] 接收到加载录像请求 - app: {}, stream: {}, date: {}, clientIP: {}",
                app, stream, date, request.getRemoteAddr());

        DeferredResult<WVPResult<StreamContent>> result = new DeferredResult<>();

        result.onTimeout(() -> {
            log.warn("[控制器-云录像加载] 加载录像文件超时 - app: {}, stream: {}, date: {}", app, stream, date);
        ...
        ErrorCallback<StreamInfo> callback = (code, msg, streamInfo) -> {
            log.info("[控制器-云录像加载] 收到回调 - app: {}, stream: {}, date: {}, code: {}, msg: {}",
                    app, stream, date, code, msg);
            ...
            if (code == InviteErrorCode.SUCCESS.getCode()) {
                ...
                if (streamInfo != null) {
                    log.info("[控制器-云录像加载] 成功获取流信息 - app: {}, stream: {}, mediaServerId: {}, fmp4: {}, ts: {}",
                            streamInfo.getApp(), streamInfo.getStream(),
                            streamInfo.getMediaServer() != null ? streamInfo.getMediaServer().getId() : "unknown",
                    ...
                    if (userSetting.getUseSourceIpAsStreamIp()) {
                        ...
                        log.debug("[控制器-云录像加载] 已更换流IP为源IP: {}", host);
                    }
                    if (!org.springframework.util.ObjectUtils.isEmpty(streamInfo.getMediaServer().getTranscodeSuffix())
                            && !"null".equalsIgnoreCase(streamInfo.getMediaServer().getTranscodeSuffix())) {
                        streamInfo.setStream(
                                streamInfo.getStream() + "_" + streamInfo.getMediaServer().getTranscodeSuffix());
                        log.debug("[控制器-云录像加载] 已添加转码后缀: {}", streamInfo.getMediaServer().getTranscodeSuffix());
                    }
                    wvpResult.setData(new StreamContent(streamInfo));
                } else {
                    log.warn("[控制器-云录像加载] 流信息为空 - app: {}, stream: {}, date: {}", app, stream, date);
                    wvpResult.setCode(code);
                    wvpResult.setMsg(msg);
                }
            } else {
                log.error("[控制器-云录像加载] 加载失败 - app: {}, stream: {}, date: {}, code: {}, msg: {}",
                        app, stream, date, code, msg);
                wvpResult.setCode(code);
                wvpResult.setMsg(msg);
            }
        ...
        try {
            ...
            log.info("[控制器-云录像加载] 已调用服务层加载录像 - app: {}, stream: {}, date: {}", app, stream, date);
        } catch (Exception e) {
            log.error("[控制器-云录像加载] 调用服务层失败 - app: {}, stream: {}, date: {}, error: {}",
                    app, stream, date, e.getMessage(), e);
            throw e;
        }
    ...
    public void seekRecord(
        ...
        log.info("[控制器-云录像定位] 接收到定位请求 - mediaServerId: {}, app: {}, stream: {}, seek: {}ms, schema: {}",
                mediaServerId, app, stream, seek, schema);
        ...
        try {
            ...
            log.info("[控制器-云录像定位] 定位请求处理完成 - mediaServerId: {}, app: {}, stream: {}, seek: {}ms",
                    mediaServerId, app, stream, seek);
        } catch (Exception e) {
            log.error("[控制器-云录像定位] 定位请求处理失败 - mediaServerId: {}, app: {}, stream: {}, seek: {}ms, error: {}",
                    mediaServerId, app, stream, seek, e.getMessage(), e);
            throw e;
        }
    }
    ...
    public void setRecordSpeed(
            ...
            @RequestParam(required = false) String schema) {
        log.info("[控制器-云录像倍速] 接收到倍速设置请求 - mediaServerId: {}, app: {}, stream: {}, speed: {}x, schema: {}",
        ...
        try {
            ...
            log.info("[控制器-云录像倍速] 倍速设置请求处理完成 - mediaServerId: {}, app: {}, stream: {}, speed: {}x",
                    mediaServerId, app, stream, speed);
        } catch (Exception e) {
            log.error("[控制器-云录像倍速] 倍速设置请求处理失败 - mediaServerId: {}, app: {}, stream: {}, speed: {}x, error: {}",
    ...
    public void downloadZipFile(HttpServletResponse response, @RequestParam(required = false) String query,
            @RequestParam(required = false) String app, @RequestParam(required = false) String stream,
            @RequestParam(required = false) String startTime, @RequestParam(required = false) String endTime,
            @RequestParam(required = false) String mediaServerId, @RequestParam(required = false) String callId,
        ...
        List<CloudRecordItem> cloudRecordItemList = cloudRecordService.getAllList(query, app, stream, startTime,
                endTime, mediaServers, callId, ids);
        ...
        try {
            ...
            for (CloudRecordItem cloudRecordItem : cloudRecordItemList) {
                zos.putNextEntry(new ZipEntry(
                        DateUtil.timestampMsToUrlToyyyy_MM_dd_HH_mm_ss((long) cloudRecordItem.getStartTime())
                                + ".mp4"));
            ...
            log.error(
                    "[下载指定录像文件的压缩包] 失败： 查询 app->{}, stream->{}, mediaServerId->{}, startTime->{}, endTime->{}, callId->{}",
    ...
    @Parameter(name = "mediaServerId", description = "流媒体ID，置空则查询全部流媒体", required = false)
    @Parameter(name = "callId", description = "每次录像的唯一标识，置空则查询全部流媒体", required = false)
    public PageInfo<CloudRecordUrl> getListWithUrl(HttpServletRequest request,
            @RequestParam(required = false) String query, @RequestParam(required = false) String app,
            @RequestParam(required = false) String stream, @RequestParam int page, @RequestParam int count,
            @RequestParam(required = false) String startTime, @RequestParam(required = false) String endTime,
            @RequestParam(required = false) String mediaServerId, @RequestParam(required = false) String callId,
        ...
        log.info(
                "[云端录像] 查询URL app->{}, stream->{}, mediaServerId->{}, page->{}, count->{}, startTime->{}, endTime->{}, callId->{}",
                app, stream, mediaServerId, page, count, startTime, endTime, callId);
        ...
        PageInfo<CloudRecordItem> cloudRecordItemPageInfo = cloudRecordService.getList(page, count, query, app, stream,
                startTime, endTime, mediaServers, callId, null);
        ...
        if (!ObjectUtils.isEmpty(cloudRecordItemPageInfo)) {
            ...
            for (CloudRecordItem cloudRecordItem : cloudRecordItemList) {
                ...
                cloudRecordUrl.setDownloadUrl(remoteHost + "/index/api/downloadFile?file_path="
                        + cloudRecordItem.getFilePath() + "&save_name=" + cloudRecordItem.getStream() + "_"
                        + cloudRecordItem.getCallId() + "_"
                        + DateUtil.timestampMsToUrlToyyyy_MM_dd_HH_mm_ss((long) cloudRecordItem.getStartTime()));
                cloudRecordUrl
                        .setPlayUrl(remoteHost + "/index/api/downloadFile?file_path=" + cloudRecordItem.getFilePath());
