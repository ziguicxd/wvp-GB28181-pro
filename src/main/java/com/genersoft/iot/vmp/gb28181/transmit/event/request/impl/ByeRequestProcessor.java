...
public class ByeRequestProcessor extends SIPRequestProcessorParent implements InitializingBean, ISIPRequestProcessor {
	...
	public void process(RequestEvent evt) {
		...
		if (sendRtpItem != null) {
			...
			log.info("[收到bye] 来自{}，停止通道：{}, 类型： {}, callId: {}", sendRtpItem.getTargetId(), channel.getGbDeviceId(),
					sendRtpItem.getPlayType(), callIdHeader.getCallId());
			...
			if (sendRtpItem.getPlayType().equals(InviteStreamType.PUSH)) {
				...
				if (platform != null) {
					...
					if (!userSetting.getServerId().equals(sendRtpItem.getServerId())) {
						...
						if (mediaServer != null) {
							mediaServerService.stopSendRtp(mediaServer, sendRtpItem.getApp(), sendRtpItem.getStream(),
									sendRtpItem.getSsrc());
			...
			if (sendRtpItem.getServerId().equals(userSetting.getServerId())) {
				...
				if (mediaServer != null) {
					...
					if (audioBroadcastCatch != null && audioBroadcastCatch.getSipTransactionInfo().getCallId()
					...
					if (mediaInfo != null && mediaInfo.getReaderCount() <= 0) {
						...
						if (sendRtpItem.getPlayType().equals(InviteStreamType.PLAY)) {
							...
							try {
								...
								cmder.streamByeCmd(device, deviceChannel.getDeviceId(), sendRtpItem.getApp(),
										sendRtpItem.getStream(), null, null);
		...
		log.info("[收到bye] 来自：{}, 通道: {}, 类型： {}", ssrcTransaction.getDeviceId(), ssrcTransaction.getChannelId(),
		...
		if (ssrcTransaction.getPlatformId() != null) {
			...
			if (ssrcTransaction.getType().equals(InviteSessionType.BROADCAST)) {
				log.info("[收到bye] 上级停止语音对讲，来自：{}, 通道已停止推流: {}", ssrcTransaction.getPlatformId(),
						ssrcTransaction.getChannelId());
				CommonGBChannel channel = channelService.getOne(ssrcTransaction.getChannelId());
				if (channel == null) {
					log.info("[收到bye] 未找到通道，上级：{}， 通道：{}", ssrcTransaction.getPlatformId(),
							ssrcTransaction.getChannelId());
					return;
				}
				String mediaServerId = ssrcTransaction.getMediaServerId();
				platformService.stopBroadcast(platform, channel, ssrcTransaction.getApp(), ssrcTransaction.getStream(),
						false,
				...
				case DOWNLOAD:
					...
					if (inviteInfo != null) {
						...
						if (inviteInfo.getStreamInfo() != null) {
							mediaServerService.closeRTPServer(inviteInfo.getStreamInfo().getMediaServer(),
