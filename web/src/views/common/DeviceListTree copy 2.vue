...
export default {
  ...
  data() {
    return {
      ...
      deviceStatus: {
        gbHasResults: false,
        pushHasResults: false,
        proxyHasResults: false
      }      
    }
  },
  ...
  watch: {
    deviceStatus: {
      ...
      handler(newVal) {
        this.$nextTick(() => {
          ['gb', 'push', 'proxy'].forEach(type => {
            const key = `${type}HasResults`;
            if (newVal[key]) {
  ...
  methods: {
    ...
    loadProxyDevices(resolve) {
      ...
      this.$store.dispatch('streamProxy/queryList', queryParams)
        .then((data) => {
          const proxyList = data.list || []; // 确保每个元素存在且包含 id 属性
    ...
    processGbDeviceData(data, resolve) {
      ...
      this.$set(this.deviceStatus, 'gbHasResults', hasResults);
    ...
    processPushDeviceData(data, resolve) {
      const hasResults = data.list?.length > 0;
      this.$set(this.deviceStatus, 'pushHasResults', hasResults);
      if (data?.records) {
    ...
    processProxyDeviceData(data, resolve) {
      const hasResults = data.list?.length > 0;
      this.$set(this.deviceStatus, 'proxyHasResults', hasResults);
    ...
    async handleGlobalSearch() {
      const originallyExpanded = new Set(this.expandedNodes);
      // 重置所有设备类型的分页信息
      this.deviceTypes.forEach(type => {
        ...
        if (node) {
          node.childNodes = [];
          node.loaded = false;
          this.expandedNodes.delete(type); // 先强制折叠      
        }
      });

      // 并发查询三个类型的第一页数据
      await Promise.all([
        this.searchDeviceType('gb'),
        this.searchDeviceType('push'),
        this.searchDeviceType('proxy')
      ]);
      // 自动展开有结果的设备类型
      this.$nextTick(() => {
        this.deviceTypes.forEach(type => {
          ...
          if (node?.childNodes?.length > 0) {
            node.expand();
            this.expandedNodes.add(type);
          } else if (originallyExpanded.has(type)) {
            node.expand(); // 恢复原本展开状态
          }
        });
      });
    },

    // 搜索指定类型设备，自动翻页直到有数据或无数据
    async searchDeviceType(deviceType) {
      ...
      return new Promise((resolve) => {
        const loadingResolver = (children) => {
          const hasResults = children?.length > 0;
          this.$set(this.deviceStatus, `${deviceType}HasResults`, hasResults);
        ...
        switch (deviceType) {
          case 'gb':
            this.loadGbDevices(loadingResolver);
            break;
          case 'push':
            this.loadPushDevices(loadingResolver);
            break;
          case 'proxy':
            this.loadProxyDevices(loadingResolver);
            break;
        }
      });
    },
